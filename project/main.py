...
def signup():
    ...
    if request.method == 'GET':
        ...
        if (patient is None) and (password == form.Re_password.data):
            ...
            BD = form.BD.data
            patient=Patient(NID=NID,username=username,password=password,mail=email,Fname=Fname,Lname=Lname,BD=BD)
            db.session.add(patient)
            db.session.commit() 
            value = f'user is {username} <br> UID is {NID} <br> mail is {email} <br> Fname is {Fname} <br> Lname is {Lname} <br> Birthday is {BD} ' 
            return render_template('out.html',output = value, Statues = "This User is added ")
        else:
            return render_template('signup.html',form = form,value="This id is aready IN")
...
def search():
    ...
    if request.method == 'GET':
        ...
        value = f'User is {patient.username} <br> UID is {patient.NID} <br> Mail is {patient.mail}'
        return render_template('out.html',output = value)
        
    else:
        raise 'There are error here'
